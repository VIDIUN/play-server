...
if (typeof String.prototype.exec != 'function') {
	String.prototype.exec = function(callback, errorCallback) {
		...
		var childProcess = child_process.exec(cmd, function (error, stdout, stderr) {
			VidiunLogger.log('Command: ' + cmd);
			VidiunLogger.debug('Standard output: ' + stdout);
			
			if(stderr.length){
				VidiunLogger.log('Standard error: ' + stderr);
			}
		...
		VidiunLogger.debug('Started cli process [' + childProcess.pid + ']: ' + cmd);
	};
}
...
VidiunUtils = {
	...
	seconds2npt: function( sec, show_ms ) {
		...
		var tm = VidiunUtils.seconds2Measurements( sec );
	...
	downloadMultiHttpUrls : function(urls, filePaths, successCallback, errorCallback) {
		...
		if(!filePaths){
			...
			for(var i = 0; i < urls.length; i++){
				filePaths[i] = VidiunConfig.config.cloud.sharedBasePath + '/tmp/' + VidiunUtils.getUniqueId();
			}
		}
		...
		for(var i = 0; i < urls.length; i++){
			VidiunUtils.downloadHttpUrl(urls[i], {filePath: filePaths[i]}, singleSuccessCallback, errorCallback);
		}
	},
	...
	getHttpUrl : function(urlStr, headers, successCallback, errorCallback) {
			...
			response.on('end', function() {
				if(response.statusCode != 200){
					if(errorCallback){
						...
						VidiunLogger.error('Invalid http status: ' + response.statusCode + ' while trying to fetch ' + urlStr);
						return;
					}
				}
				if(successCallback){
		...
		request.on('error', function(e) {
			if(errorCallback){
				...
				VidiunLogger.error('Error received on HTTP call: [' + urlStr  + ']');
			}
		});
	...
	md5OnFile : function(urlStr, headers, bytes, successCallback, errorCallback) {
		VidiunLogger.log('Calculate md5 for [' + urlStr + '] bytes [' + bytes + ']');
		var parsedUrl = url.parse(urlStr);
		var httpModule = VidiunUtils.getHttpModuleByProtocol(parsedUrl.protocol);
		var options = {
			hostname : parsedUrl.hostname,
		...
		var request = httpModule.request(options, function(response) {
			...
			response.on('end', function() {
				...
				if(successCallback){
					VidiunLogger.log('Downloaded [' + length + '] bytes, md5 calculated for [' + actualMd5Length + '] bytes');
	...
	buildFilePath: function(baseDir, fileName){
		...
		return path.join(VidiunConfig.config.cloud.sharedBasePath, baseDir, dir1, dir2, fileName);
	},
		...
		mkdirp(dirName, function (err) {
		    if (err){
		    	if(errorCallback){
		    		...
		    		VidiunLogger.error('Failed to create dir path: ' + err);
		    	}
		    }
		    else{
		    	successCallback();
	...
	downloadHttpUrl : function(urlStr, options, successCallback, errorCallback) {
		...
		VidiunLogger.debug('Starting download [' + urlStr + ']');
		
		var watchFileOnFs = function(filePath, retries, successCallback, errorCallback){
			VidiunLogger.log('Watching file download to: [' + filePath + '] retries left' + '[' + retries + ']');
			setTimeout(function(){
				fs.exists(filePath, function(exists){
					if(exists){
						VidiunLogger.log('File from url [' + urlStr + '], already downloaded to [' + filePath + ']');
		...
		fs.exists(options.filePath, function(exists){
			if(exists){
				VidiunLogger.log('File from url [' + urlStr + '], already downloaded to [' + options.filePath + ']');
				successCallback(options.filePath);
			}
			else{
				var fileDownloadingKey = VidiunCache.getKey(VidiunCache.FILE_DOWNLOADING_KEY_PREFIX, [options.filePath]);
				VidiunCache.add(fileDownloadingKey, true, VidiunConfig.config.cache.fileDownloadTimeout, function(){										
					tempPath = VidiunConfig.config.cloud.sharedBasePath + '/tmp/' + VidiunUtils.getUniqueId();	
					VidiunLogger.log('File from url [' + urlStr + '], downloading to temp path[' + tempPath + ']');
					...
					var request = httpModule.request(options, function(response) {
						...
						localFile.on('finish', function() {
							localFile.close();
							VidiunUtils.createFilePath(options.filePath, function(){
								VidiunLogger.log('Moving temp file[' + tempPath + '] to [' + options.filePath + ']');
					...
					VidiunLogger.log('File from url [' + urlStr + '] already downloading');
					var retries = VidiunConfig.config.cache.fileDownloadTimeout / DOWNLOAD_RETRY_INTERVAL;
	...
	downloadHttpUrlForce: function(urlStr, downloadToBuffer, downloadToFile, callback, errorCallback){		
		...
		var request = httpModule.request(options, function(response) {
			if (response.statusCode != 200) {
				if(errorCallback){
					...
					VidiunLogger.error('Failed to download url [' + urlStr + ']: Invalid http status: ' + response.statusCode);
				}
			...
			var localPath = null;
			if(downloadToFile){				
				localPath = VidiunConfig.config.cloud.sharedBasePath + '/tmp/' + VidiunUtils.getUniqueId();
				VidiunLogger.log('Downloading from url [' + urlStr + '] to [' + localPath +']');
			...
			response.on('end', function() {
				VidiunLogger.log('Finished downloading from url [' + urlStr + ']');
				if(callback){
		...
		request.on('error', function(e) {
			if(errorCallback){
				...
				VidiunLogger.error('Failed to download url [' + urlStr + ']: http error: ' + e.message);
			}
		});
