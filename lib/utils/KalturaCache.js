...
VidiunCache = {
	...
	init: function(){
		this.config = VidiunConfig.config.memcache;
		
		this.server = new memcached(this.config.hostname + ':' + this.config.port, this.config);
		this.server.on('failure', function( details ){ VidiunLogger.error( "Server " + details.server + " went down due to: " +   details.messages.join( '' ) ) });
		this.server.on('issue',   function( details ){ VidiunLogger.error( "Server " + details.server + " encountered issues: " + details.messages.join( '' ) ) });
	...
	getBinServerByKey : function(key){
		var serverId = this.hashString(key) % this.binservers.length;
                VidiunLogger.log("id for key (" + key + ") = " + serverId);
	...
	getManifestId : function(manifestUrl) {
		if(VidiunConfig.config.cache.hackWowzaUniqueSession && parseInt(VidiunConfig.config.cache.hackWowzaUniqueSession)){
	...
	removeRenditionIdFromEntryRequiredValue: function(entryRequired, renditionId, retries){
		...
		if (VidiunConfig.config.cache.lock) {
			lockTimeout = VidiunConfig.config.cache.lock;
		}
		var recall = function() {
			setTimeout(function () {
		...
		this.add(lockKey, 1, lockTimeout, function() { // add fails if key exists
			VidiunLogger.debug("Removing " + renditionId + " from " + entryRequired + " values");
			This.get(entryRequired, function (data) {
				if (!data){
					VidiunLogger.log("EntryRequired value was null - deleting key");
					...
					if ( index != -1) {
						...
						if (newEntryRequired.trim().length > 0) {
							This.set(entryRequired, newEntryRequired, VidiunConfig.config.cache.entryRequired,
								function () {
									This.del(lockKey);
									VidiunLogger.debug("Successfully removed rendition " + renditionId + " from entryRequired " + entryRequired +
										" new value is : " + newEntryRequired);
								},
								function () {
									This.del(lockKey);
									VidiunLogger.debug("Failed to remove rendition " + renditionId + " from entryRequired " + entryRequired +
										" ,Though should have - retrying");
									recall();
								});
						} else {
							VidiunLogger.debug("After deletion of rendition " + renditionId + " entryRequired: " + entryRequired + " is empty - deleting it");
							This.del(entryRequired);
							This.del(lockKey);
						}
					} else {
				...
				VidiunLogger.error("Failed to get " + entryRequired + " key while trying to remove rendition :" + renditionId);
			});
		}, function(){
			VidiunLogger.debug("Failed to get lock for deleting rendition " + renditionId + " from entryRequired : " + entryRequired + " retrying");
			if (!retries)
				retries = 0;
			if (retries < This.MAX_RETRIES_FOR_REND_DELETE)
				recall();
			else {
				VidiunLogger.error("Failed to remove rendition id " + renditionId + " from entryRequired: " + entryRequired);
			}
		});
	...
	incr : function(key, value, callback, errorCallback) {
		...
		this.server.incr(key, value, function(err, data){
			if(err){
				...
				VidiunLogger.debug('Cache.incr [' + key + ']: OK', stackSource);
				if(callback)
					callback(data);
			}
		});
	...
	get : function(key, callback, errorCallback) {
		var stackSource = this.getStack();
		VidiunLogger.debug('Cache.get [' + key + ']...', stackSource);
		var This = this;
		This.server.get(key, function(err, data){
			if(err){
				...
				VidiunLogger.debug('Cache.get [' + key + ']: OK', stackSource);
				if(callback)
					callback(data);
			} 
		});
	...
	set : function(key, value, lifetime, callback, errorCallback) {
		...
		this.server.set(key, value, lifetime, function(err){
			if(err){
				...
				VidiunLogger.debug('Cache.set [' + key + ']: OK', stackSource);
				if(callback)
					callback();
			}
		});
	...
	touch : function(key, lifetime, callback, errorCallback) {
		...
		var cacheTouchCallback = function(err, isNullValue){
			if(err){
				...
				VidiunLogger.debug('Cache.touch [' + key + ']: OK', stackSource);
				if(callback)
					callback();
			}
		};
	...
	add : function(key, value, lifetime, callback, errorCallback) {
		...
		this.server.add(key, value, lifetime, function(err){
			if(err){
				...
				VidiunLogger.debug('Cache.add [' + key + ']: OK', stackSource);
				if(callback)
					callback();
			}
		});
	...
	append : function(key, value, callback, errorCallback) {
		...
		this.server.append(key, value, function(err){
			if(err){
				...
				VidiunLogger.debug('Cache.append [' + key + ']: OK', stackSource);
				if(callback)
					callback();
			}
		});
	...
	getMulti : function(keys, callback, errorCallback) {
		...
		keys.forEach(function(key, index, array){
			VidiunCache.server.get(key, function(err, data){
    			if(err){
    				...
    				VidiunLogger.debug('Cache.getMulti [' + key + ']: OK', stackSource);
    				answers[key] = data;
        			missingAnswers--;
	...
	del : function(key, callback, errorCallback) {
		...
		return this.server.del(key, function(err){
			if(err){
				...
				VidiunLogger.debug('Cache.del [' + key + ']: OK', stackSource);
				if(callback)
					callback();
			}
		});
	...
	replace : function(key, value, lifetime, callback, errorCallback) {
		...
		return this.server.replace(key, value, lifetime, function(err){
			if(err){
				...
				VidiunLogger.debug('Cache.replace [' + key + ']: OK', stackSource);
				if(callback)
					callback();
			}
		});
	...
	getBinary : function(key, callback, errorCallback) {
		var stackSource = this.getStack();
		VidiunLogger.debug('Cache.getBinary [' + key + ']...', stackSource);
		...
		memcacheServer.get(key, function(err, data){
			if(err){
				...
				if (data)
					VidiunLogger.debug('Cache.getBinary [' + key + '], value size [' + data.length + '] took ' + took + ' miliseconds: OK', stackSource);
				else
                                        VidiunLogger.debug('Cache.getBinary [' + key + '], value is null, took ' + took + ' miliseconds', stackSource);
    			...
    			if(err){
    				...
    				if(errorCallback){
    					...
    					VidiunLogger.error(errMessage + "\n" + stackSource.stack, stackSource);
                                ...
                                var took = Date.now() - timeInMs;
                                if (data)
                                        VidiunLogger.debug('Cache.getMultiBinary [' + key + '], value size [' + data.length + '] took ' + took + ' miliseconds: OK', stackSource);
                                else
                                        VidiunLogger.debug('Cache.getMultiBinary [' + key + '], value is null, took ' + took + ' miliseconds', stackSource);
