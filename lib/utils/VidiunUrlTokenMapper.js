...
VidiunUrlTokenMapper = {
	
	// ability to format expressions
	formatFunctions : {
		timeFormat : function(value) {
			return VidiunUtils.seconds2npt(parseFloat(value));
		},
	...
	mapDynamicTokens: function(request, cachedUrl, playerConfig){
		...
		for(var attributeName in queryString){
			var attributeValue = VidiunUrlTokenMapper.evaluate(data, queryString[attributeName]);
			VidiunLogger.log("Evaluated attribute: [" + attributeName + "] value: [" + attributeValue + "]");		
	...
	mapFixedTokens: function(request, cuePoint, entry, metadata, playerConfig, callback){
		...
		for(var attributeName in queryString){
			var attributeValue = VidiunUrlTokenMapper.evaluate(data, queryString[attributeName], defaultTokenValue);
			VidiunLogger.log("Evaluated attribute: [" + attributeName + "] value: [" + attributeValue + "]");
		...
		VidiunLogger.log('Saving cue point url in cache: [' + cachedUrl + ']');
		VidiunCache.set(cuePointUrlKey, cachedUrl, VidiunConfig.config.cache.cuePoint);
	...
	*/
	evaluate : function(data, objectString, defaultTokenValue, limit) {
		...
		limit = limit || 0;
		if (limit > 4) {
			VidiunLogger.log('recursive calls are limited to 5');
			return objectString;
		}
