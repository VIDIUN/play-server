...
var VidiunMediaInfo = {
	...
	config: {
		general: {
			bitrate: {
				...
				callback: function(value){
					return VidiunMediaInfo.parseBitrate(value);
				}
			}
		},
		video: {
			duration: {
				names: ['duration'],
				callback: function(value){
					return VidiunMediaInfo.parseDuration(value);
				}
			},
			bitrate: {
			...
			width: {
				names: ['width'],
				callback: function(value){
					return VidiunMediaInfo.parseItem(value, ['pixels']);
				}
			},
			height: {
				names: ['height'],
			...
			frameRate: {
				names: ['frame rate'],
				callback: function(value){
					return VidiunMediaInfo.parseItem(value, ['fps']);
				}
			},
			reframes: {
				names: ['format settings, reframes'],
				callback: function(value){
					return VidiunMediaInfo.parseItem(value, ['frame', 'frames']);
				}
			},
			profile: {
				names: ['format profile'],
				callback: function(value){
					return VidiunMediaInfo.parseVideoProfile(value);
				}
			},			
			colorPrimaries: {
		...
		audio: {
			...
			sampleRate: {
				names: ['sampling rate'],
				callback: function(value){
					return VidiunMediaInfo.parseItem(value, ['khz']) * 1000;
				}
			},
			channels: {
				names: ['channel(s)'],
				callback: function(value){
					return VidiunMediaInfo.parseItem(value, ['channel', 'channels']);
				}
			},
			profile: {
				names: ['format profile'],
				callback: function(value){
					return VidiunMediaInfo.parseAudioProfile(value);
				}
			},
		}		
	},
	...
	parseRaw: function(data, callback){
		...
		for(sectionName in VidiunMediaInfo.config){
			if(!parsedAttributes[sectionName]){
				continue;
			}
			
			mediaInfo[sectionName] = {};
			var section = VidiunMediaInfo.config[sectionName];
	...
	parse: function(filePath, callback){
		...
		fs.stat(filePath, function (err, stats) {
			...
			child_process.exec(cmd, function (error, stdout, stderr) {
				var mediaInfo = VidiunMediaInfo.parseRaw(stdout);
...
module.exports = VidiunMediaInfo;
